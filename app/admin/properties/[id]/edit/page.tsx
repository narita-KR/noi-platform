...
function getMockProperty(id: string) {
  return {
    ...
    city: "港区赤坂",
    address: "1-2-3",
    building: "赤坂タワー 301号室",
    accessPoints: [
      { line: "東京メトロ千代田線", station: "赤坂駅", walkMinutes: "3" },
      { line: "東京メトロ銀座線", station: "溜池山王駅", walkMinutes: "5" },
    ],
    builtYear: "2005",
    builtMonth: "3",
    ...
    floors: "14",
    landArea: "1250.00",
    buildingArea: "8500.50",
    fullIncome: "360",
    currentIncome: "320",
    appraisal: "4500",
    mainImage: { id: "main-1", name: "main_photo.jpg" },
    subImages: [
      { id: "sub-1", name: "exterior_01.jpg" },
      { id: "sub-2", name: "interior_01.jpg" },
      { id: "sub-3", name: "floorplan.jpg" },
    ],
    description:
      "赤坂駅徒歩3分の好立地に位置する区分マンションです。2005年築のRC造14階建て、管理体制良好。現在満室稼働中で安定した収益が見込めます。",
    facilities: ["エアコン", "オートロック", "宅配ボックス", "エレベーター", "モニター付きインターホン"],
    remarks: "管理費：月額12,000円、修繕積立金：月額8,500円",
    sellerId: "1",
    contactName: "山田 一郎",
    contactPhone: "03-1234-5678",
    updatedAt: "2026/01/29 15:30",
  };
}
...
export default function EditPropertyPage({
  ...
  /* --- Load existing data --- */
  useEffect(() => {
    const data = getMockProperty(id);
    setTitle(data.title);
    setCategory(data.category);
    setStatus(data.status);
    setPrice(data.price);
    setSurfaceYield(data.surfaceYield);
    ...
    setBuiltYear(data.builtYear);
    setBuiltMonth(data.builtMonth);
    setStructure(data.structure);
    setFloors(data.floors);
    setLandArea(data.landArea);
    setBuildingArea(data.buildingArea);
    setFullIncome(data.fullIncome);
  ...
  const addAccessPoint = () => {
    setAccessPoints((prev) => [...prev, { line: "", station: "", walkMinutes: "" }]);
  };
  ...
  const removeSubImage = (imgId: string) => {
    ...
    <div className="pb-24">
      ...
      <div className="mb-6 space-y-3">
        ...
        <nav className="flex items-center gap-1.5 text-xs text-gray-400">
          <Link href="/admin" className="text-gray-500 no-underline hover:text-blue-600">
            ダッシュボード
          </Link>
          <span>/</span>
          <Link href="/admin/properties" className="text-gray-500 no-underline hover:text-blue-600">
            物件管理
          </Link>
          <span>/</span>
          <span className="max-w-48 truncate text-gray-700">{title}</span>
        </nav>

        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">物件編集</h1>
            {updatedAt && (
      ...
      <div className="space-y-5">
        ...
        <Section title="基本情報" tag="A">
          <div className="grid gap-5 sm:grid-cols-2">
            <div className="sm:col-span-2">
              <Label required htmlFor="prop-title">物件名</Label>
              <input
                id="prop-title"
                type="text"
                value={title}
            ...
            <div>
              <Label required htmlFor="prop-price">価格</Label>
              <div className="flex items-center gap-2">
                <input
                  ...
                  className={inputCls}
                />
                <span className="shrink-0 text-sm font-medium text-gray-500">万円</span>
              </div>
            ...
            <div>
              ...
              {calculatedYield && !surfaceYield && (
                <p className="mt-1 text-xs text-blue-500">自動計算: {calculatedYield}%</p>
              )}
            </div>
          </div>
        </Section>
        ...
        <Section title="所在地" tag="B">
          <div className="grid gap-5 sm:grid-cols-2">
            ...
            <div>
              <Label required htmlFor="prop-prefecture">都道府県</Label>
              <select
                id="prop-prefecture"
                value={prefecture}
              ...
              {errors.prefecture && <p className={errorCls}>{errors.prefecture}</p>}
            </div>

            <div>
              <Label required htmlFor="prop-city">市区町村</Label>
              <input
                id="prop-city"
                type="text"
                value={city}
            ...
            <div>
              <Label required htmlFor="prop-address">番地</Label>
              <input
                id="prop-address"
                type="text"
                value={address}
              ...
              >
                ...
                <div>
                  ...
                  <input
                    ...
                    type="text"
                    value={ap.station}
                    onChange={(e) => updateAccessPoint(i, "station", e.target.value)}
                    placeholder="長原駅"
                    className={inputCls}
                  />
                </div>
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      onChange={(e) => updateAccessPoint(i, "walkMinutes", e.target.value)}
                      placeholder="5"
                      className={inputCls}
                      min={0}
                    />
                    <span className="shrink-0 text-sm font-medium text-gray-500">分</span>
                  </div>
                </div>
              </div>
            ))}
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  min={1}
                />
                <span className="shrink-0 text-sm font-medium text-gray-500">階建</span>
              </div>
            </div>
          ...
          <div className="grid gap-5 sm:grid-cols-2">
            ...
            <div>
              ...
              <div className="flex h-11 items-center rounded-md border border-gray-200 bg-gray-50 px-3 text-sm">
                {calculatedYield ? (
                  <span className="font-semibold text-blue-600">{calculatedYield}%</span>
                ) : (
                  <span className="text-gray-400">価格と満室想定年収を入力すると自動計算</span>
                )}
              </div>
            </div>
            ...
            <div>
              ...
              {mainImage ? (
                <div className="flex items-center gap-3 rounded-lg border border-gray-200 bg-gray-50 p-3">
                  ...
                  <span className="flex-1 text-sm text-gray-600">{mainImage.name}</span>
                  <button
                    type="button"
                ...
                >
                  ...
                  <span className="text-sm font-medium">クリックまたはドラッグ＆ドロップで画像をアップロード</span>
      ...
      <div className="fixed inset-x-0 bottom-0 z-30 border-t border-gray-200 bg-white px-6 py-3 shadow-lg lg:left-64">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <button
            type="button"
            onClick={handleDelete}
            className="inline-flex items-center gap-1.5 rounded-md border border-red-200 px-5 py-2.5 text-sm font-medium text-red-600 transition-colors hover:bg-red-50"
          >
            <Trash2 className="h-4 w-4" />
            削除
          </button>
          ...
          <div className="flex flex-wrap items-center gap-3">
            <Link
              href="/admin/properties"
              className="rounded-md border border-gray-300 px-5 py-2.5 text-sm font-medium text-gray-600 no-underline transition-colors hover:bg-gray-50"
            >
              キャンセル
            </Link>
            <button
